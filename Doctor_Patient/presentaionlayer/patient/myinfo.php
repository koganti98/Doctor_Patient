<?php include ('C:\xampp\htdocs\Doctor_Patient\datalayer\server.php'); ?>
<!DOCTYPE html>
<html>
<head>
    <title>Patient</title>
...
<body>
    ...
    <form method="post" action="myinfo.php" class="infoP" style="margin-left: -1px; margin-top: 0px; width: 40%; padding: 20px; border: 3px solid #39ca74; background: white; border-radius: 10px 10px 10px 10px;">
        ...
        <div class="contentP" style="font-weight: bold;">
            <label>ID: <?php echo $_SESSION['UserID'] ?? ''; ?></label><br><br>
            <label>Email: <?php echo $col['Email'] ?? ''; ?></label><br><br>
            <label>Name: <?php echo $col['Name'] ?? ''; ?></label><br><br>
            <label>Address: <?php echo $col['Address'] ?? ''; ?></label><br><br>
            <label>Contact Number: <?php echo $col['ContactNumber'] ?? ''; ?></label><br><br>
            <label>Blood Type: <?php echo $col['Bloodtype'] ?? ''; ?></label><br><br>
    ...
    if (isset($_POST['treatmentHistory'])) {
        ?>
        <table class="table2">
            <caption style="margin-left: 24px; padding: 10px; font-weight: bold; font-size: 20px;" class="asd">Treatment History</caption>
            <tr>
                <th>DoctorID</th>
                <th>DoctorName</th>
                <th>Treatment</th>
            ...
            <?php
            $PID = $mysqli->real_escape_string($_SESSION['UserID']);
            ...
            if (mysqli_num_rows($resultP2) >= 1) {
                ...
                echo "<tr><td colspan='4'>No treatment history available.</td></tr>";
            }
        }
        ?>
    </table>
